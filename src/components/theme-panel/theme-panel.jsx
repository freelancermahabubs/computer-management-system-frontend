  ...
  const setAppTheme = (event, theme) => {
    ...
    for (var x = 0; x < document.body.classList.length; x++) {
      if (
        ...
        document.body.classList[x] !== theme
      ) {
          ...
          <Card className="mb-3">
            <CardBody className="p-2">
              <div className="row gx-2">
                {modeList.length > 0 &&
                  modeList.map((m, index) => (
                    <div className="col-6" key={index}>
                      <a
                        href="#/"
                        onClick={(event) => setAppMode(event, m.value)}
                        className={
                          "app-theme-mode-link " +
                          (mode === m.value ? "active" : "")
                        }>
                ...
                {themeList.length > 0 &&
                  themeList.map((t, index) => (
                    <div
                      className={
                        "app-theme-list-item " +
                        (theme === t.themeClass ? "active" : "")
                      }
                      key={index}>
                      <a
                        href="#/"
                        onClick={(event) => setAppTheme(event, t.themeClass)}
                        className={"app-theme-list-link " + t.bgClass}
          ...
          <Card>
            <CardBody className="p-2">
              <div className="app-theme-cover">
                {coverList?.length > 0 &&
                  coverList.map((c, index) => (
                    <div
                      className={
                        "app-theme-cover-item " +
                        (cover === c.coverClass ? "active" : "")
                      }
                      key={index}>
                      <a
                        href="#/"
                        onClick={(event) => setAppCover(event, c.coverClass)}
                        className="app-theme-cover-link"
                        style={{
                          backgroundImage: "url(" + c.coverThumbImage + ")",
                        }}
                        data-cover-class={c.coverClass}
                        data-bs-toggle="tooltip"
                        data-bs-trigger="hover"
                        data-bs-container="body">
                        &nbsp;
